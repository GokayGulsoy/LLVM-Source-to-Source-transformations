  ...
  bool VisitCallExpr(CallExpr *CE) {
    if (isCUDAKernelCall(CE)) {
      ...
      if (!BeginLoc.isInvalid() && !EndLoc.isInvalid()) {
 	  ...
 	  int reductionRatioint = stof(ThreadReductionRatio); 
 	 
 	  cout <<  ThreadValueint * reductionRatioint << endl;
 	  int reducedThreadValue = ThreadValueint - ThreadValueint * reductionRatioint;
 	  cout << reducedThreadValue << endl;
