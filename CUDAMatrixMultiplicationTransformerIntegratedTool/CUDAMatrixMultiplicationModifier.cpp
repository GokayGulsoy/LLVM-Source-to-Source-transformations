...
cl::opt<std::string>
    ThreadsValue("threads",
                 ...
                 cl::value_desc("value"), cl::init("64"));
...
public:
  ...
  // Visit variable declarations and change the THREADS value.
  bool VisitVarDecl(VarDecl *VD) {
    if (VD->getNameAsString() == "THREADS") {
      ...
      if (VD->hasInit()) {
        ...
        StringRef ReplacementText = ThreadsValue;
  ...
  bool VisitTypeLoc(TypeLoc TL) {
    if (ConvertDoubleToFloat) {
      ...
      if (QT->isSpecificBuiltinType(BuiltinType::Double)) {
        ...
        // Calculate the correct replacement length to ensure adjacent words are
        // separated.
...
public:
  ...
  void HandleTranslationUnit(ASTContext &Context) override {
    // Visit and transform variable declarations.
...
int main(int argc, const char **argv) {
  ...
  // Create a FrontendActionFactory for MyFrontendAction.
